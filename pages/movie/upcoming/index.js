...
// domain.com/movie/upcoming
const Upcoming = (props) => {
  const router = useRouter();

  useEffect(() => {
    useNavstore.setState({ routepath: '/movie/upcoming' });
    router.prefetch('/movie/upcoming');
  }, []);
...
export const getServerSideProps = async ({ req, res, query }) => {
  ...
  const movies = await getDataPage('/movie/upcoming', `${page}`);
